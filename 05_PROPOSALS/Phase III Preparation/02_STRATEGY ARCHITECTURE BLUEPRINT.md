...
- `11_PROJECTS/` — **empty** except `.gitkeep`. No active project workspaces.
...
- `13_OPERATIONS/` — mature cadence (Weekly/Monthly/Quarterly/Annual, Review Cycles, Dashboards) — the execution end already works.
...
| L7 Quarterly Portfolios | 3 mo | *Which projects this quarter* | `13_OPERATIONS/Quarterly Reviews` (exists) | Exec Council | Quarterly | planned/active/closed |
| L8 Active Projects | weeks–mo | *What is being built* | `11_PROJECTS/` | Project owner | Weekly | active/blocked/done |
| L9 Milestones | per project | *Proof points* | inside each project | Project owner | Weekly | pending/hit/slipped |
| L10 Weekly Execution | 1 wk | *This week's moves* | `13_OPERATIONS/Weekly Operations` (exists) | Delegates | Weekly | rolling |
| L11 Daily Focus | 1 day | *Today* | `13_OPERATIONS/Daily Operations` (exists) | Delegates | Daily | rolling |

**Non-duplication mechanism:** L3–L6 live as **distinct notes in `02_STRATEGY/`**, each linking *up* (parent objective) and *down* (child items). No level copies its parent's text; it links to it. L7/L10/L11 already exist in `13_OPERATIONS` — the blueprint **connects to them, does not recreate them.**
...
| `11_PROJECTS/_Project Template.md` | L8/L9 | Every project proves alignment (see below) |

## Required Databases / Indexes

- **Strategy Cascade Index** (Dataview) — one query rendering L1→L11 with status roll-up.
- **Alignment Ledger** — each `11_PROJECTS` note carries `parent_objective:` (a link to its L4/L5 parent). A Dataview query lists any project with a missing/broken `parent_objective` → **strategic-drift detector**.
...
- Cadence already has homes in `13_OPERATIONS/Review Cycles` — the blueprint plugs strategy reviews into the existing cadence rather than inventing a parallel one.
...
1. **Orphan projects:** `11_PROJECTS` note lacking a valid `parent_objective` link.
...
Every `11_PROJECTS` note's frontmatter carries `parent_objective:` + a one-line "alignment claim." A project **cannot enter `active`** without a valid parent link (enforced by review, later by validator). This is the concrete bridge the vault currently lacks.
...
- **Fact:** `02_STRATEGY` and `11_PROJECTS` are empty; vision and cadence exist and are disconnected.
