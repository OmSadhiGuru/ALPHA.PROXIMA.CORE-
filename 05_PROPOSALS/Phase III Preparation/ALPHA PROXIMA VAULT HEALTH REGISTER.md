...
| H9 | **Folder numbering collisions** | `ls`: `06_GOVERNANCE`+`11_PROJECTS`; `09_FUTURE`+`12_PEOPLE` | those 4 folders | 🟠 | Ordinal taxonomy broken; two folders per prefix confuses routing/sort | Renumber one of each pair (migration note required per doctrine) | — | ⚠️ needs migration note |
