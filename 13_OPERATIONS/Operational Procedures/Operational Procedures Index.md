---
title: "Operational Procedures Index"
aliases: ["Operations Procedures"]
tags: [operations, procedures, alpha-proxima]
created: 2026-07-02
updated: 2026-07-02
status: active
version: "1.0.0"
authors: ["CODEX"]
artifact_type: operations-index
institutional_owner: "Alpha Proxima Foundation"
cognitive_function: "Implementation"
reasoning_engine: "CODEX"
dependencies: ["[[13_OPERATIONS]]", "[[Workflow Registry]]"]
related_documents: ["[[13_OPERATIONS]]", "[[Workflow Registry]]"]
related_research_programs: []
---

# Operational Procedures Index

## Purpose

Index reusable procedures that help offices operate consistently.

## Scope

Procedures describe repeatable operations. They do not decide outcomes.

## Dependencies

- [[13_OPERATIONS]]
- [[Workflow Registry]]

## Version

| Field | Value |
|-------|-------|
| **Version** | 1.0.0 |
| **Status** | active |

## Author

[[CODEX]]

## Related Documents

- [[Review Cycles Registry]]
- [[Artifact Registry]]

## Related Research Programs

- N/A

## Implementation Notes

New procedures should be created only when they reduce repeated coordination work.

## Future Improvements

- [ ] Add procedure template.
- [ ] Add procedure status dashboard.

## Core Content

| Procedure | Status | Purpose |
|-----------|--------|---------|
| Review packet preparation | planned | Prepare daily, weekly, monthly, quarterly, and annual packets |
| Proposal routing | planned | Route future proposals into research, architecture, engineering, decision, or archive |
| Engineering validation run | planned | Run and review validators |

## Version History

| Version | Date | Author | Summary |
|---------|------|--------|---------|
| 1.0.0 | 2026-07-02 | [[CODEX]] | Initial procedures index |

