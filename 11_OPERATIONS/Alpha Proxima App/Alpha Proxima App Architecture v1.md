...
The seven uncanonical folders are the sharper finding. `06_GOVERNANCE` and `governance` both exist while `06_PROJECTS` also claims the `06` prefix; `09_PEOPLE` and `09_FUTURE` both claim `09`; `ALPHA PROXIMA/` (28 documents), `OSG_LAUNCH/` (29), `OSG_BUSINESS/` (13), `docs/`, and 8 loose root files sit outside [[03 - Folder Naming Convention]] entirely. That is 81 documents — 30% of the Foundation — living outside its own declared hierarchy.
