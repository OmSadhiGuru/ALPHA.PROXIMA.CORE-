---
title: "Node Registry Report"
aliases: ["Institutional Knowledge Graph Node Registry Report"]
tags: [systems, engineering, knowledge-graph, node-registry, report, alpha-proxima]
created: 2026-09-04
updated: 2026-09-04
status: draft
version: "1.0.0"
authors: ["CODEX"]
artifact_type: engineering-report
institutional_owner: "Alpha Proxima Foundation"
cognitive_function: "Implementation"
reasoning_engine: "CODEX"
dependencies: ["[[Tool 010 - Node Registry Generator]]", "[[Node Taxonomy]]"]
related_documents: ["[[Engineering Program EP-001 - Institutional Knowledge Graph]]", "[[Knowledge Graph Architecture v1.0]]"]
related_research_programs: []
---

# Node Registry Report

## Purpose

Summarize graph node candidates discovered by [[Tool 010 - Node Registry Generator]].

## Summary

- Vault: `/Users/Fred/Documents/Obsidian Vault`
- Generated: `2026-09-04T09:38:36-04:00`
- Total nodes discovered: `373`
- Unknown node types: `174`
- Missing YAML: `18`
- High-value cleanup candidates: `247`

## Nodes by Type

| Node Type | Count |
|-----------|-------|
| `unknown` | `174` |
| `standard` | `39` |
| `engineering_tool` | `19` |
| `future_research` | `19` |
| `concept` | `16` |
| `research_artifact` | `16` |
| `founder_directive` | `14` |
| `charter` | `13` |
| `standing_order` | `13` |
| `policy` | `12` |
| `theory` | `9` |
| `office` | `8` |
| `research_program` | `7` |
| `canonical_synthesis` | `4` |
| `open_question` | `4` |
| `evidence_claim` | `3` |
| `person` | `2` |
| `research_commission` | `1` |

## Unknown Node Types

| Source Path | Title | Reason |
|-------------|-------|--------|
| `00_CONSTITUTION/Constitutional Hierarchy Statement.md` | Constitutional Hierarchy Statement | no_confident_rule |
| `00_CONSTITUTION/Founding Principles of Alpha Proxima.md` | Founding Principles of Alpha Proxima | no_confident_rule |
| `01_VISION/Alpha Proxima — 10 Year Vision.md` | Alpha Proxima — 10 Year Vision | no_confident_rule |
| `03_AI_COUNCIL/AI Council Registry.md` | AI Council Registry | no_confident_rule |
| `03_AI_COUNCIL/Alpha Council.md` | Alpha Council | no_confident_rule |
| `03_AI_COUNCIL/Cognitive Function Matrix.md` | Cognitive Function Matrix | no_confident_rule |
| `03_AI_COUNCIL/Cognitive Function Registry.md` | Cognitive Function Registry | no_confident_rule |
| `03_AI_COUNCIL/Engine Registry.md` | Engine Registry | no_confident_rule |
| `03_AI_COUNCIL/Engine Succession Policy.md` | Engine Succession Policy | no_confident_rule |
| `03_AI_COUNCIL/Institutional Registry.md` | Institutional Registry | no_confident_rule |
| `04_DECISIONS/ADR-0001 - The Founding Decision.md` | ADR-0001 - The Founding Decision | no_confident_rule |
| `04_DECISIONS/ADR-0002 - Reconciling the Four Institutional Taxonomies.md` | ADR-0002 - Reconciling the Four Institutional Taxonomies | no_confident_rule |
| `06_GOVERNANCE/Canonical Terminology/Canonical Terminology Register.md` | Canonical Terminology Register | no_confident_rule |
| `06_GOVERNANCE/Constitutional Audit/CAR-001 Constitutional Audit Report.md` | CAR-001 Constitutional Audit Report | no_confident_rule |
| `06_GOVERNANCE/Constitutional Impact Report/CIR-001 Epoch III Constitutional Refactoring.md` | CIR-001 — Epoch III Constitutional Refactoring | no_confident_rule |
| `06_GOVERNANCE/Constitutional Impact Report/CIR-002 Institutional Completeness Review.md` | CIR-002 — Institutional Completeness Review | no_confident_rule |
| `06_GOVERNANCE/Constitutional Impact Report/CIR-003 Epoch V Constitutional Coherence.md` | CIR-003 Epoch V — Constitutional Coherence | no_confident_rule |
| `06_GOVERNANCE/Directive Governance Framework/Directive Governance Framework.md` | Directive Governance Framework | no_confident_rule |
| `06_GOVERNANCE/Epoch V/Book II Amendment — Council Topology.md` | Book II Amendment — Council Topology | no_confident_rule |
| `06_GOVERNANCE/Epoch V/Governance Model Crosswalk.md` | Governance Model Crosswalk & Council Topology | no_confident_rule |
| `06_GOVERNANCE/Epoch V/Interim Authority Instrument.md` | Interim Authority Instrument | no_confident_rule |
| `06_GOVERNANCE/Foundation Gap Report/FGR-001 Epoch II Stewardship Audit.md` | FGR-001 — Epoch II Stewardship Audit | no_confident_rule |
| `06_GOVERNANCE/Institutional Glossary & Acronym Register.md` | Institutional Glossary & Acronym Register | no_confident_rule |
| `06_GOVERNANCE/Institutional Policies/Citation Policy.md` | Citation Policy | no_confident_rule |
| `06_GOVERNANCE/Institutional Policies/Metadata Policy.md` | Metadata Policy | no_confident_rule |
| `06_GOVERNANCE/Institutional Policies/Naming Policy.md` | Naming Policy | no_confident_rule |
| `06_GOVERNANCE/Institutional Policies/Privacy Policy.md` | Privacy Policy | no_confident_rule |
| `06_GOVERNANCE/Institutional Policies/Source Attribution Policy.md` | Source Attribution Policy | no_confident_rule |
| `06_GOVERNANCE/Institutional Policies/Versioning Policy.md` | Versioning Policy | no_confident_rule |
| `06_GOVERNANCE/Institutional Timeline.md` | Institutional Timeline | no_confident_rule |
| `06_GOVERNANCE/Research Debt Register/Research Debt Register.md` | Research Debt Register | no_confident_rule |
| `06_GOVERNANCE/Research Framework/Institutional Intelligence Translation Framework v1.0.md` | Institutional Intelligence Translation Framework v1.0 | no_confident_rule |
| `06_GOVERNANCE/Research Framework/Research Integration Framework.md` | Research Integration Framework | no_confident_rule |
| `06_GOVERNANCE/Research Framework/Research Program Playbook v1.0.md` | Research Program Playbook v1.0 | no_confident_rule |
| `06_GOVERNANCE/Research Framework/Translation Checklist.md` | Translation Checklist | no_confident_rule |
| `06_GOVERNANCE/Research Framework/Translation Decision Matrix.md` | Translation Decision Matrix | no_confident_rule |
| `06_GOVERNANCE/Research Framework/Translation Review Process.md` | Translation Review Process | no_confident_rule |
| `06_GOVERNANCE/Research Methodology/Alpha Proxima Research Methodology v1.0.md` | Alpha Proxima Research Methodology v1.0 | no_confident_rule |
| `06_GOVERNANCE/Standards Council/Standards Council Evaluation.md` | Standards Council — Institutional Evaluation | no_confident_rule |
| `06_GOVERNANCE/Standing Orders/SO-001 Institutional Observatory — Continuous Monitoring Protocol.md` | SO-001 — Institutional Observatory: Continuous Monitoring Protocol | no_confident_rule |
| `06_GOVERNANCE/Standing Orders/Standing Orders Register.md` | Standing Orders Register | no_confident_rule |
| `07_RESEARCH/RP-001/00 Executive Summary/RP-001 Executive Summary.md` | RP-001 Executive Summary | no_confident_rule |
| `07_RESEARCH/RP-001/01 Research Question/RP-001 Research Question.md` | RP-001 Research Question | no_confident_rule |
| `07_RESEARCH/RP-001/02 Objectives/RP-001 Objectives.md` | RP-001 Objectives | no_confident_rule |
| `07_RESEARCH/RP-001/03 Source Registry/RP-001 Source Registry.md` | RP-001 Source Registry | no_confident_rule |
| `07_RESEARCH/RP-001/08 Comparative Framework/RP-001 Comparative Framework.md` | RP-001 Comparative Framework | no_confident_rule |
| `07_RESEARCH/RP-001/10 Theory Matrix/RP-001 Theory Matrix.md` | RP-001 Consciousness Theory Matrix | no_confident_rule |
| `07_RESEARCH/RP-001/11 Canonical Glossary/RP-001 Canonical Glossary.md` | RP-001 Canonical Glossary | no_confident_rule |
| `07_RESEARCH/RP-001/13 Research Graph/RP-001 Research Graph.md` | RP-001 Research Graph | no_confident_rule |
| `07_RESEARCH/RP-001/16 Visual Knowledge/RP-001 Visual Knowledge Index.md` | RP-001 Visual Knowledge Index | no_confident_rule |
| `07_RESEARCH/RP-001/17 NotebookLM Package/RP-001 NotebookLM Source Pack.md` | RP-001 NotebookLM Source Pack | no_confident_rule |
| `07_RESEARCH/RP-001/18 Related Constitution/RP-001 Constitutional Links.md` | RP-001 Constitutional Links | no_confident_rule |
| `07_RESEARCH/RP-001/19 Related Laws/RP-001 Governing Provisions.md` | RP-001 Governing Provisions | no_confident_rule |
| `07_RESEARCH/RP-001/20 Related ADRs/RP-001 ADR Links.md` | RP-001 ADR Links | no_confident_rule |
| `07_RESEARCH/RP-001/21 Version History/RP-001 Version History.md` | RP-001 Version History | no_confident_rule |
| `07_RESEARCH/RP-002/00 Executive Summary/RP-002 Executive Summary.md` | RP-002 Executive Summary — Atlas of Human Memory | no_confident_rule |
| `07_RESEARCH/RP-002/01 Research Question/RP-002 Research Question.md` | RP-002 Research Question | no_confident_rule |
| `07_RESEARCH/RP-002/02 Objectives/RP-002 Objectives.md` | RP-002 Objectives | no_confident_rule |
| `07_RESEARCH/RP-002/03 Source Registry/RP-002 Source Registry.md` | RP-002 Source Registry | no_confident_rule |
| `07_RESEARCH/RP-002/08 Comparative Framework/RP-002 Comparative Framework.md` | RP-002 Comparative Framework | no_confident_rule |
| `07_RESEARCH/RP-002/10 Theory Matrix/RP-002 Theory Matrix.md` | RP-002 Theory Matrix | no_confident_rule |
| `07_RESEARCH/RP-002/11 Canonical Glossary/RP-002 Canonical Glossary.md` | RP-002 Canonical Glossary | no_confident_rule |
| `07_RESEARCH/RP-002/13 Research Graph/RP-002 Research Graph.md` | RP-002 Research Graph | no_confident_rule |
| `07_RESEARCH/RP-002/16 Visual Knowledge/RP-002 Visual Knowledge Index.md` | RP-002 Visual Knowledge Index | no_confident_rule |
| `07_RESEARCH/RP-002/17 NotebookLM Package/RP-002 NotebookLM Source Pack.md` | RP-002 NotebookLM Source Pack | no_confident_rule |
| `07_RESEARCH/RP-002/18 Related Constitution/RP-002 Constitutional Links.md` | RP-002 Constitutional Links | no_confident_rule |
| `07_RESEARCH/RP-002/19 Related Laws/RP-002 Governing Provisions.md` | RP-002 Governing Provisions | no_confident_rule |
| `07_RESEARCH/RP-002/20 Related ADRs/RP-002 ADR Links.md` | RP-002 ADR Links | no_confident_rule |
| `07_RESEARCH/RP-002/21 Version History/RP-002 Version History.md` | RP-002 Version History | no_confident_rule |
| `07_RESEARCH/RP-003/00 Institutional Stewardship Review/ISR-001 Institutional Synthesis Report.md` | ISR-001 — Institutional Synthesis Report: RP-001 and RP-002 | no_confident_rule |
| `07_RESEARCH/RP-003/00 Institutional Stewardship Review/ISR-001 Knowledge Graph Update Recommendations.md` | ISR-001 Knowledge Graph Update Recommendations | no_confident_rule |
| `08_SYSTEMS/ALPHAPROXIMA Enterprise Knowledge Architecture v1.0.md` | ALPHAPROXIMA Enterprise Knowledge Architecture v1.0 | no_confident_rule |
| `08_SYSTEMS/Alpha Proxima Operating Model v1.0.md` | Alpha Proxima Operating Model v1.0 | no_confident_rule |
| `08_SYSTEMS/Automation/Vault Note Generator.md` | Vault Note Generator | no_confident_rule |
| `08_SYSTEMS/Engineering Toolkit/OSG Reading Color CSS Guide.md` | OSG Reading Color CSS Guide | no_confident_rule |
| `08_SYSTEMS/Foundational Architecture.md` | Foundational Architecture | no_confident_rule |
| `08_SYSTEMS/Institutional Relationship Map.md` | Institutional Relationship Map | no_confident_rule |
| `08_SYSTEMS/Knowledge Architecture Specification.md` | Knowledge Architecture Specification | no_confident_rule |
| `08_SYSTEMS/LUMIAION Architecture Spec v0.1.md` | LUMIAION Architecture Spec v0.1 | no_confident_rule |
| `08_SYSTEMS/The Orchestration Framework.md` | The Orchestration Framework | no_confident_rule |
| `08_SYSTEMS/Visual Systems/Color System - Implementation Checklist.md` | Color System - Implementation Checklist | no_confident_rule |
| `09_OFFICES/Research Intelligence Office/Research Office Matrix.md` | Research Office Matrix | no_confident_rule |
| `10_TEMPLATES/ADR Template.md` | ADR Template | no_confident_rule |
| `10_TEMPLATES/Concept Note Template.md` | Concept Note Template | no_confident_rule |
| `10_TEMPLATES/Implementation Note Template.md` | Implementation Note Template | no_confident_rule |
| `10_TEMPLATES/Institutional Translation Template v1.0.md` | Institutional Translation Template v1.0 | no_confident_rule |
| `10_TEMPLATES/Research Commission Template v2.0.md` | Research Commission Template v2.0 | no_confident_rule |
| `10_TEMPLATES/Research Note Template.md` | [Research Title] | no_confident_rule |
| `10_TEMPLATES/Research Program Template/Research Program Methodology.md` | Research Program Methodology — Alpha Proxima Standard | no_confident_rule |
| `10_TEMPLATES/Vault Structure Convention.md` | Vault Structure Convention | no_confident_rule |
| `11_PROJECTS/README.md` | 11_PROJECTS | no_confident_rule |
| `13_OPERATIONS/Alpha Proxima App/Alpha Proxima App Architecture v1.md` | Alpha Proxima App Architecture v1 | no_confident_rule |
| `13_OPERATIONS/Reboot/Repository Reboot Audit - 2026-08-23.md` | Repository Reboot Audit - 2026-08-23 | no_confident_rule |
| `13_OPERATIONS/Workflow Registry/LUMIAION - Operating Manual (LOOM).md` | LUMIAION Operating Manual (LOOM) | no_confident_rule |
| `99_ARCHIVE/Legacy ALPHA PROXIMA/ALPHA.PROXIMA.FOUNDATION/Building achitecture/ALPHA PROXIMA ROLES/AI COUNCIL/AI COUNCIL.md` | AI Council | no_confident_rule |
| `99_ARCHIVE/Legacy ALPHA PROXIMA/ALPHA.PROXIMA.FOUNDATION/Building achitecture/ALPHA PROXIMA ROLES/ARCHITECTURE MAP.md` | Architecture Map | no_confident_rule |
| `99_ARCHIVE/Legacy ALPHA PROXIMA/ALPHA.PROXIMA.FOUNDATION/Building achitecture/LUMIAION VAULT.md` | LUMIAION Vault | no_confident_rule |
| `99_ARCHIVE/Legacy ALPHA PROXIMA/ALPHA.PROXIMA.FOUNDATION/building milestone/BUILDING MILESTONE.md` | Building Milestone | no_confident_rule |
| `99_ARCHIVE/Legacy ALPHA PROXIMA/ALPHA.PROXIMA.FOUNDATION/building milestone/Phase 1/CONSTITUTIONV1.0.md` | CONSTITUTIONV1.0 | no_confident_rule |
| `99_ARCHIVE/Legacy ALPHA PROXIMA/ALPHA.PROXIMA.FOUNDATION/building milestone/Phase 1/PHASE 1 - FOUNDATION.md` | Phase 1 - Foundation | no_confident_rule |

## Missing YAML

| Source Path | Title |
|-------------|-------|
| `Awaken the Inner Guru Production Folder.md` | Awaken the Inner Guru Production Folder |
| `OSG_BUSINESS/00 OSG Business Foundation — Overview.md` | OSG Business Foundation — Launch Package |
| `OSG_BUSINESS/01 Flagship Course.md` | Flagship Course — *The Inner Architecture* |
| `OSG_BUSINESS/02 Coaching Offers.md` | OSG Coaching Offers |
| `OSG_BUSINESS/03 Website Copy.md` | OSG Website Copy |
| `OSG_BUSINESS/04 Client Journey & Onboarding.md` | Client Journey & Onboarding |
| `OSG_BUSINESS/05 Email Sequences.md` | Email Sequences |
| `OSG_BUSINESS/06 Community Onboarding.md` | Community Onboarding Flow |
| `OSG_BUSINESS/07 30-Day Launch Checklist.md` | 30-Day Launch Checklist |
| `OSG_BUSINESS/OSG_ACADEMY/Awaken the Inner Guru — Production Blueprint.md` | Awaken the Inner Guru |
| `OSG_BUSINESS/OSG_ACADEMY/OSG Learning Standard (OLS) v1.0.md` | OSG Learning Standard (OLS) v1.0 |
| `Omi/Memories.md` | Omi Memory Export |
| `Sans titre 1.md` | Sans titre 1 |
| `Sans titre.md` | Sans titre |
| `Vault.md` | Vault |
| `docs/constitution/LUMIAION_CONSTITUTION.md` | LUMIAION |
| `docs/constitution/README.md` | LUMIAION Constitution |
| `docs/setup/Claude-Code-in-Obsidian.md` | Claude Code inside Obsidian — Setup Guide |

## High-Value Cleanup Candidates

| Source Path | Issue |
|-------------|-------|
| `00_CONSTITUTION/Constitutional Hierarchy Statement.md` | unknown node type, missing artifact_type |
| `00_CONSTITUTION/Founding Principles of Alpha Proxima.md` | unknown node type, missing artifact_type |
| `01_VISION/Alpha Proxima — 10 Year Vision.md` | unknown node type, missing artifact_type |
| `03_AI_COUNCIL/AI Council Registry.md` | unknown node type, missing artifact_type |
| `03_AI_COUNCIL/Alpha Council.md` | unknown node type, missing artifact_type |
| `03_AI_COUNCIL/Cognitive Council Charter.md` | missing artifact_type |
| `03_AI_COUNCIL/Cognitive Function Matrix.md` | unknown node type, missing artifact_type |
| `03_AI_COUNCIL/Cognitive Function Registry.md` | unknown node type, missing artifact_type |
| `03_AI_COUNCIL/Departments/ATHENA Charter.md` | missing artifact_type |
| `03_AI_COUNCIL/Departments/JERANIUM Charter.md` | missing artifact_type |
| `03_AI_COUNCIL/Departments/LUMIAION Charter.md` | missing artifact_type |
| `03_AI_COUNCIL/Departments/SOHMA Charter.md` | missing artifact_type |
| `03_AI_COUNCIL/Departments/VORTEX Charter.md` | missing artifact_type |
| `03_AI_COUNCIL/Engine Registry.md` | unknown node type, missing artifact_type |
| `03_AI_COUNCIL/Engine Succession Policy.md` | unknown node type, missing artifact_type |
| `03_AI_COUNCIL/Institutional Registry.md` | unknown node type, missing artifact_type |
| `03_AI_COUNCIL/YUNA Charter.md` | missing artifact_type |
| `04_DECISIONS/ADR-0001 - The Founding Decision.md` | unknown node type, missing artifact_type |
| `04_DECISIONS/ADR-0002 - Reconciling the Four Institutional Taxonomies.md` | unknown node type, missing artifact_type |
| `05_PROPOSALS/CN-0001 - Constitutional Alignment Gap Report.md` | missing artifact_type |
| `06_GOVERNANCE/Canonical Terminology/Canonical Terminology Register.md` | unknown node type, missing artifact_type |
| `06_GOVERNANCE/Constitutional Audit/CAR-001 Constitutional Audit Report.md` | unknown node type, missing artifact_type |
| `06_GOVERNANCE/Constitutional Impact Report/CIR-001 Epoch III Constitutional Refactoring.md` | unknown node type, missing artifact_type |
| `06_GOVERNANCE/Constitutional Impact Report/CIR-002 Institutional Completeness Review.md` | unknown node type, missing artifact_type |
| `06_GOVERNANCE/Constitutional Impact Report/CIR-003 Epoch V Constitutional Coherence.md` | unknown node type |
| `06_GOVERNANCE/Directive Governance Framework/Directive Governance Framework.md` | unknown node type, missing artifact_type |
| `06_GOVERNANCE/Epoch V/Book II Amendment — Council Topology.md` | unknown node type |
| `06_GOVERNANCE/Epoch V/Governance Model Crosswalk.md` | unknown node type, missing artifact_type |
| `06_GOVERNANCE/Epoch V/Interim Authority Instrument.md` | unknown node type, missing artifact_type |
| `06_GOVERNANCE/Foundation Gap Report/FGR-001 Epoch II Stewardship Audit.md` | unknown node type, missing artifact_type |
| `06_GOVERNANCE/Founder Directives/Founder Directives Register.md` | missing artifact_type |
| `06_GOVERNANCE/Institutional Glossary & Acronym Register.md` | unknown node type, missing artifact_type |
| `06_GOVERNANCE/Institutional Open Questions Register.md` | missing artifact_type |
| `06_GOVERNANCE/Institutional Policies/Citation Policy.md` | unknown node type, missing artifact_type |
| `06_GOVERNANCE/Institutional Policies/Metadata Policy.md` | unknown node type, missing artifact_type |
| `06_GOVERNANCE/Institutional Policies/Naming Policy.md` | unknown node type, missing artifact_type |
| `06_GOVERNANCE/Institutional Policies/Privacy Policy.md` | unknown node type, missing artifact_type |
| `06_GOVERNANCE/Institutional Policies/Source Attribution Policy.md` | unknown node type, missing artifact_type |
| `06_GOVERNANCE/Institutional Policies/Versioning Policy.md` | unknown node type, missing artifact_type |
| `06_GOVERNANCE/Institutional Timeline.md` | unknown node type, missing artifact_type |
| `06_GOVERNANCE/Research Debt Register/Research Debt Register.md` | unknown node type, missing artifact_type |
| `06_GOVERNANCE/Research Framework/Institutional Intelligence Translation Framework v1.0.md` | unknown node type, missing artifact_type |
| `06_GOVERNANCE/Research Framework/Research Integration Framework.md` | unknown node type, missing artifact_type |
| `06_GOVERNANCE/Research Framework/Research Program Playbook v1.0.md` | unknown node type, missing artifact_type |
| `06_GOVERNANCE/Research Framework/Translation Checklist.md` | unknown node type, missing artifact_type |
| `06_GOVERNANCE/Research Framework/Translation Decision Matrix.md` | unknown node type, missing artifact_type |
| `06_GOVERNANCE/Research Framework/Translation Review Process.md` | unknown node type, missing artifact_type |
| `06_GOVERNANCE/Research Methodology/Alpha Proxima Research Methodology v1.0.md` | unknown node type, missing artifact_type |
| `06_GOVERNANCE/Standards Council/Standards Council Evaluation.md` | unknown node type, missing artifact_type |
| `06_GOVERNANCE/Standing Orders/SO-001 Institutional Observatory — Continuous Monitoring Protocol.md` | unknown node type, missing artifact_type |
| `06_GOVERNANCE/Standing Orders/Standing Orders Register.md` | unknown node type, missing artifact_type |
| `07_RESEARCH/RP-001/00 Executive Summary/RP-001 Executive Summary.md` | unknown node type, missing artifact_type |
| `07_RESEARCH/RP-001/01 Research Question/RP-001 Research Question.md` | unknown node type, missing artifact_type |
| `07_RESEARCH/RP-001/02 Objectives/RP-001 Objectives.md` | unknown node type, missing artifact_type |
| `07_RESEARCH/RP-001/03 Source Registry/RP-001 Source Registry.md` | unknown node type, missing artifact_type |
| `07_RESEARCH/RP-001/04 Source - Perplexity/RP-001 Source Note - Perplexity.md` | missing artifact_type |
| `07_RESEARCH/RP-001/05 Source - Gemini/RP-001 Source Note - Gemini.md` | missing artifact_type |
| `07_RESEARCH/RP-001/06 Source - SanaLab/RP-001 Source Note - SanaLab.md` | missing artifact_type |
| `07_RESEARCH/RP-001/07 Future Sources/RP-001 Future Sources.md` | missing artifact_type |
| `07_RESEARCH/RP-001/08 Comparative Framework/RP-001 Comparative Framework.md` | unknown node type, missing artifact_type |
| `07_RESEARCH/RP-001/10 Theory Matrix/RP-001 Theory Matrix.md` | unknown node type, missing artifact_type |
| `07_RESEARCH/RP-001/11 Canonical Glossary/RP-001 Canonical Glossary.md` | unknown node type, missing artifact_type |
| `07_RESEARCH/RP-001/13 Research Graph/Concepts/4E Cognition.md` | missing artifact_type |
| `07_RESEARCH/RP-001/13 Research Graph/Concepts/Active Inference.md` | missing artifact_type |
| `07_RESEARCH/RP-001/13 Research Graph/Concepts/Attention Schema Theory.md` | missing artifact_type |
| `07_RESEARCH/RP-001/13 Research Graph/Concepts/Consciousness.md` | missing artifact_type |
| `07_RESEARCH/RP-001/13 Research Graph/Concepts/Default Mode Network.md` | missing artifact_type |
| `07_RESEARCH/RP-001/13 Research Graph/Concepts/Global Neuronal Workspace Theory.md` | missing artifact_type |
| `07_RESEARCH/RP-001/13 Research Graph/Concepts/Higher-Order Thought Theory.md` | missing artifact_type |
| `07_RESEARCH/RP-001/13 Research Graph/Concepts/Illusionism.md` | missing artifact_type |
| `07_RESEARCH/RP-001/13 Research Graph/Concepts/Integrated Information Theory.md` | missing artifact_type |
| `07_RESEARCH/RP-001/13 Research Graph/Concepts/Meta Awareness.md` | missing artifact_type |
| `07_RESEARCH/RP-001/13 Research Graph/Concepts/Neural Correlates of Consciousness.md` | missing artifact_type |
| `07_RESEARCH/RP-001/13 Research Graph/Concepts/Orchestrated Objective Reduction.md` | missing artifact_type |
| `07_RESEARCH/RP-001/13 Research Graph/Concepts/Panpsychism.md` | missing artifact_type |
| `07_RESEARCH/RP-001/13 Research Graph/Concepts/Predictive Processing.md` | missing artifact_type |
| `07_RESEARCH/RP-001/13 Research Graph/Concepts/Recurrent Processing Theory.md` | missing artifact_type |
| `07_RESEARCH/RP-001/13 Research Graph/RP-001 Research Graph.md` | unknown node type, missing artifact_type |
| `07_RESEARCH/RP-001/14 Open Questions/RP-001 Open Questions.md` | missing artifact_type |
| `07_RESEARCH/RP-001/15 Future Experiments/RP-001 Future Research Opportunities.md` | missing artifact_type |
| `07_RESEARCH/RP-001/16 Visual Knowledge/RP-001 Visual Knowledge Index.md` | unknown node type, missing artifact_type |
| `07_RESEARCH/RP-001/17 NotebookLM Package/RP-001 NotebookLM Source Pack.md` | unknown node type, missing artifact_type |
| `07_RESEARCH/RP-001/18 Related Constitution/RP-001 Constitutional Links.md` | unknown node type, missing artifact_type |
| `07_RESEARCH/RP-001/19 Related Laws/RP-001 Governing Provisions.md` | unknown node type, missing artifact_type |
| `07_RESEARCH/RP-001/20 Related ADRs/RP-001 ADR Links.md` | unknown node type, missing artifact_type |
| `07_RESEARCH/RP-001/21 Version History/RP-001 Version History.md` | unknown node type, missing artifact_type |
| `07_RESEARCH/RP-001/ARCHIVE/ARCHIVE Philosophy.md` | missing artifact_type |
| `07_RESEARCH/RP-001/ARCHIVE/DOC-001 Architecture of Human Consciousness.md` | missing artifact_type |
| `07_RESEARCH/RP-001/ARCHIVE/DOC-003 Comparative Framework - SanaLab.md` | missing artifact_type |
| `07_RESEARCH/RP-001/ARCHIVE/DOC-004 GNWT vs IIT Deep Dive - SanaLab.md` | missing artifact_type |
| `07_RESEARCH/RP-002/00 Executive Summary/RP-002 Executive Summary.md` | unknown node type, missing artifact_type |
| `07_RESEARCH/RP-002/01 Research Question/RP-002 Research Question.md` | unknown node type, missing artifact_type |
| `07_RESEARCH/RP-002/02 Objectives/RP-002 Objectives.md` | unknown node type, missing artifact_type |
| `07_RESEARCH/RP-002/03 Source Registry/RP-002 Source Registry.md` | unknown node type, missing artifact_type |
| `07_RESEARCH/RP-002/04 Source - CORE-002/RP-002 Source Note - CORE-002.md` | missing artifact_type |
| `07_RESEARCH/RP-002/05 Source - SanaLab/RP-002 Source Note - SanaLab.md` | missing artifact_type |
| `07_RESEARCH/RP-002/06 Source - Illustrated/RP-002 Source Note - Illustrated.md` | missing artifact_type |
| `07_RESEARCH/RP-002/07 Future Sources/RP-002 Future Sources.md` | missing artifact_type |
| `07_RESEARCH/RP-002/08 Comparative Framework/RP-002 Comparative Framework.md` | unknown node type, missing artifact_type |
| `07_RESEARCH/RP-002/09 Canonical Synthesis/RP-002 Canonical Synthesis.md` | missing artifact_type |

## Recommendations for ES-006

- Build the Relationship Extractor using this registry as the node lookup table.
- Convert wiki links into candidate `REFERENCES` relationships.
- Convert YAML `dependencies` into candidate `REQUIRES` relationships.
- Preserve extraction confidence and provenance on every relationship.
- Do not promote inferred relationships to canonical status without review.

## Implementation Notes

This report is generated from a read-only Markdown scan. It does not modify notes or create canonical claims.

## Future Improvements

- [ ] Add configurable node inference rules.
- [ ] Add schema validation against [[Node Taxonomy]].
- [ ] Add stable ID override support in YAML.

## Version History

| Version | Date | Author | Summary |
|---------|------|--------|---------|
| 1.0.0 | 2026-09-04 | [[CODEX]] | Node registry report generated |
